...
使用安装后的 `scripts/render.py`、准备好的解释器及仓库中的 `examples/article.md`。以昵称「纸上散步」、默认米白背景、未显示日期生成到新的输出目录；头像可使用内置首字占位。

命令形状如下，实际执行时展开绝对路径并按操作系统正确引用：

```text
<python> <installed-skill>/scripts/render.py --article <repo>/examples/article.md --name "纸上散步" --output <new-output-directory>
```
